...
if [ "${ANYWAY_ETL_USE_LATEST_TAG}" == "yes" ]; then
  NEW_ANYWAY_ETL_COMMIT="$(curl -fs https://api.github.com/repos/hasadna/anyway-etl/releases/latest | jq -r .tag_name)"
  ANYWAY_COMMIT_URL="https://raw.githubusercontent.com/hasadna/anyway-etl/${NEW_ANYWAY_ETL_COMMIT}/anyway-${ANYWAY_BRANCH:-dev}-commit.txt"
  ANYWAY_ETL_REQUIREMENTS_URL="https://raw.githubusercontent.com/hasadna/anyway-etl/${NEW_ANYWAY_ETL_COMMIT}/requirements.txt"
  ANYWAY_ETL_INSTALL_URL="git+https://github.com/hasadna/anyway-etl@${NEW_ANYWAY_ETL_COMMIT}#egg=anyway-etl"
else
  NEW_ANYWAY_ETL_COMMIT="$(curl -fs https://api.github.com/repos/hasadna/anyway-etl/branches/${ANYWAY_ETL_BRANCH} | jq -r .commit.sha)"
  ANYWAY_COMMIT_URL="https://raw.githubusercontent.com/hasadna/anyway-etl/${ANYWAY_ETL_BRANCH}/anyway-${ANYWAY_BRANCH:-dev}-commit.txt"
  ANYWAY_ETL_REQUIREMENTS_URL="https://raw.githubusercontent.com/hasadna/anyway-etl/${ANYWAY_ETL_BRANCH}/requirements.txt"
  ANYWAY_ETL_INSTALL_URL="git+https://github.com/hasadna/anyway-etl@${ANYWAY_ETL_BRANCH}#egg=anyway-etl"
fi
