...
from ..config import ANYWAY_ETL_STATIC_DATA_ROOT_PATH
...
@functools.lru_cache
def get_localization_cities():
    return pd.read_csv(
        os.path.join(ANYWAY_ETL_STATIC_DATA_ROOT_PATH, 'cities.csv'),
        encoding="utf-8", index_col="SEMEL"
    )


def get_localization_city_name(symbol_id, lang: str = "he") -> Optional[str]:
    column_to_fetch = "NAME" if lang == "he" else "ENGLISH_NAME"
    try:
        return get_localization_cities().loc[symbol_id, column_to_fetch]
    except Exception:
        return None


def get_supported_localization_tables():
    return LOCALIZATION_TABLES.keys()


def get_localization_field(field, value=None):
    if value:
        table = LOCALIZATION_TABLES.get(field, None)
        return table.get(value, None) if table else None
    return LOCALIZATION_FIELDS.get(field, None)
